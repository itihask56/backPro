...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = User.findByIdAndUpdate(
    req.user?._id,
    {
      $set: {
        fullName: fullName,
        email, //both fullName and email are accepted as like that
      },
    },
    { new: true }
  ).select("-password");
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
  ...
  .status(200)
  .json(
    new ApiResponse(200,user,"avatar updated succfully")
  )
})
...
const updateUserCoverImage = asyncHandler(async(req,res)=>{
  ...
  .status(200)
  .json(
    new ApiResponse(200,user,"CoverImage updated succfully")
  )
})
